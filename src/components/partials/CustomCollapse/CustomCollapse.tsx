...
interface CollapseIconProps {
  ...
  const CollapseIcon:React.FC<CollapseIconProps> = ({rotate}) =>{
    return <div className={`h-6 w-6 transition-all`} style={{transform:`rotate(${rotate}deg)`}}>{IconChevronDown()}</div>
  }
...
export default function CustomCollapse(props: CustomCollapseProps) {
  ...
  return (
    <Collapse
    size="large"
    expandIcon={({ isActive }) =>{ setCollapseActive(Boolean(isActive)); return props.prependIcon ? <div className="h-6 w-6">{props.prependIcon}</div>  : undefined} }
